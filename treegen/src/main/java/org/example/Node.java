...
public class Node implements Comparable<Node>,Cloneable{
    ...
    @Override
     ...
     public Node copy() throws CloneNotSupportedException
     {
       ...
       {
//           System.out.println("zav" + this.Name);
    ...
    private void print(StringBuilder buffer, String prefix, String childrenPrefix) {
       ...
       // buffer.append(","+this.calculateCost());
        ...
        //System.out.println(this.Name + "," + this.type + "," + + this.neighbors.size());
       // System.out.println(this.neighbors.keySet());
        /*for (Iterator<Node> it = _children.iterator(); it.hasNext();) {
    ...
    public double performanceCoefficient()
    {
        ...
        while(profile_iterator.hasNext())
        {
           ...
           //System.out.println(profile_pair.getKey());
    ...
    public double getCooperation()
    {
        ...
        for(int i=0;i<this.cooperation.size();i++)
        {
            ...
            //System.out.println(this.cooperation.get(i) + " ," + two._id);
    ...
    public double pairCooperation(Node one, Node two)
    {
        ...
        //System.out.println(two.Name + " EFANTASY");
    ...
    {
       //System.out.println("den eftase edo");
        if(this.type==Type.Leaf)
        {
          ////System.out.println(this.Name);
            ...
            if(this.resource.Type.equals("Blank"))
            {
              ...
              ////System.out.println(this.Name +"," + this.resource.Name + " dialga");
            
            return (this.time * performanceCoefficient() * this.getCooperation());
           // return this.cost;
            }
        }
        else
        {
            ...
            while(it.hasNext())
            {
                Map.Entry<Integer,Node> temp = (Map.Entry<Integer,Node>) it.next();
                        Node temp_node = temp.getValue();
                       //System.out.println(this.Name + "," + " hulkengerg" + temp_node.Name + this.neighbors.size() + "," + this._children.size());
                        
                        sum += temp_node.calculateTime();
            ...
            else if(this.type==type.XOR_AND)
            {
                ...
                Map.Entry<Integer,Node> temp = (Map.Entry<Integer,Node>) it.next();
                        Node temp_node = temp.getValue();
                        if(temp_node.calculateTime()<min)
                        {
                            min=temp_node.calculateTime();
                        }
                }
            ...
            else if(this.type==type.AND)
            {
                ...
                Map.Entry<Integer,Node> temp = (Map.Entry<Integer,Node>) it.next();
                        Node temp_node = temp.getValue();
                       // System.out.println(this.Name + " GODFATHER "+ temp_node.Name + "," + temp_node.type + ", CHILDREN: " + this.neighbors.size());
                        if(temp_node.calculateTime()>max)
                        {
                            max=temp_node.calculateTime();
                        }
                }
            ...
            else if(this.type==type.XOR)
            {
               ...
               while(it.hasNext())
               {
                Map.Entry<Integer,Node> temp = (Map.Entry<Integer,Node>) it.next();
                        Node temp_node = temp.getValue();
                       //System.out.println(temp_node.parent._id + " CRAWLINKG");
                      // System.out.println(this._id  + ", "+ this.Name + "        LINKIN PARK!");
                       // System.out.println(temp.getKey() + "," + this.path.length + " beatles");
                        sum += this.path[temp.getKey()]*temp_node.calculateTime();
    ...
    {
       //System.out.println("den eftase edo" + this.Name);
        if(this.type==Type.Leaf)
        {
          //System.out.println(this.Name + "edo?");
            
            
            
              ////System.out.println(this.Name +"," + this.resource.Name + " dialga"); * performanceCoefficient() * this.getCooperation()
            
            return (this.cost   * performanceCoefficient() * this.getCooperation());
           // return this.cost;
        ...
        else
        {
            ...
            else if(this.type==type.XOR_AND)
            {
                ...
                while(it.hasNext())
                {
                // System.out.println(this.Name + " PRIN TO CRASH " + this.neighbors.size());
                Map.Entry<Integer,Node> temp = (Map.Entry<Integer,Node>) it.next();
                        Node temp_node = temp.getValue();
                        if(temp_node.calculateTime()<min)
                        {
